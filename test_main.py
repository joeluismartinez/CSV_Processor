...
def test_results_columns_equal_to_schema():
    ...
    final_columns = results.columns.values.tolist()
    assert final_columns == schema_cols


def test_results_string_datatype():
     ''' test dataframe matches expected schema datatypes for strings '''
     assert results.dtypes['Provider Name'] == pd.StringDtype
     assert results.dtypes['CampaignID'] == pd.StringDtype
     assert results.dtypes['Redirect Link'] == pd.StringDtype
     assert results.dtypes['Phone Number'] == pd.StringDtype
     assert results.dtypes['Address'] == pd.StringDtype
     assert results.dtypes['Zipcode'] == pd.StringDtype


def test_results_float64_datatype():
     ''' test dataframe matches expected schema datatype for float64 '''
     assert results.dtypes['Cost Per Ad Click'] == np.float64
